...
dataset = MOT17ObjDetect('MOT17Det/train', get_transform(train=False))
...
data_loader_no_random = torch.utils.data.DataLoader(
    dataset_no_random, batch_size=16, shuffle=False, num_workers=4,
    collate_fn=utils.collate_fn)
...
model.to(device)
#model = torch.nn.DataParallel(model)
#model_state_dict = torch.load(f"./faster_rcnn_fpn_training_mot_17/resnet50/model_epoch_27.model")
...
def evaluate_and_write_result_files(model, data_loader):
  ...
  data_loader.dataset.write_results_files(results, './no_normalized/resnet50/')
...
for epoch in range(1, num_epochs + 1):
    ...
    if epoch % 28 == 27:
      ...
      torch.save(model.state_dict(), f"./no_normalized/model_epoch_{epoch}.model")
